...
const devIconsDir = join(__dirname, "..");
const svgDir = join(devIconsDir, "icons");
const outputDir = join(devIconsDir, "components");
...
function addIconMetadata(componentName: string) {
  const componentPath = `./icons/dev-icons/components/${componentName}.vue`;
  const metadata = {
    name: componentName,
    group: "dev-icons",
    groupShortName: "di",
...
const iconFiles = generateComponents(svgDir, "Di");
...
const indexContent = [
  ...iconFiles.map((svgFile: string) => {
    const componentName = svgFile
      ...
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as Di${componentName} } from './components/Di${componentName}.vue'`;
  }),
].join("\n");

writeFileSync(join(devIconsDir, "index.ts"), indexContent);
