...
const featherIconsDir = join(__dirname, "..");
const svgDir = join(featherIconsDir, "icons");
const outputDir = join(featherIconsDir, "components");
...
function addIconMetadata(componentName: string) {
  const componentPath = `./icons/feather-icons/components/${componentName}.vue`;
  const metadata = {
    name: componentName,
    group: "feather-icons",
    groupShortName: "fi",
...
const iconFiles = generateComponents(svgDir, "Fi");
...
const indexContent = [
  ...iconFiles.map((svgFile: string) => {
    const componentName = svgFile
      ...
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as Fi${componentName} } from './components/Fi${componentName}.vue'`;
  }),
].join("\n");

writeFileSync(join(featherIconsDir, "index.ts"), indexContent);
