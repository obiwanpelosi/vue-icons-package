...
// Path configurations
const antIconsDir = join(__dirname, "..");
const filledDir = join(antIconsDir, "filled");
const outlinedDir = join(antIconsDir, "outlined");
const outputDir = join(antIconsDir, "components");
...
function addIconMetadata(componentName: string) {
  const componentPath = `./icons/ant-icons/components/${componentName}.vue`;
  const metadata = {
    name: componentName,
    group: "ant-icons",
    groupShortName: "ai",
...
const filledFiles = generateComponents(filledDir, "Ai", "Fill");
const outlinedFiles = generateComponents(outlinedDir, "Ai", "Outline");

// Generate index file
const indexContent = [
  ...filledFiles.map((svgFile: string) => {
    const componentName = svgFile
      .replace(".svg", "")
      .split(/[_&-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as AiFill${componentName} } from './components/AiFill${componentName}.vue'`;
  }),
  ...outlinedFiles.map((svgFile: string) => {
    const componentName = svgFile
      .replace(".svg", "")
      .split(/[_&-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as AiOutline${componentName} } from './components/AiOutline${componentName}.vue'`;
  }),
].join("\n");

writeFileSync(join(antIconsDir, "index.ts"), indexContent);
