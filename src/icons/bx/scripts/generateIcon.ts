...
const outputDir = join(boxIconsDir, "components");
...
function addIconMetadata(componentName: string) {
  const componentPath = `./icons/box-icons/components/${componentName}.vue`;
  const metadata = {
    name: componentName,
    group: "box-icons",
    groupShortName: "bx",
...
function generateComponents(svgDir: string, lib: string, type: string) {
  ...
  svgFiles.forEach((svgFile: string) => {
    ...
    const componentName = svgFile
      ...
      .join("");
    const componentFullName = lib + type + componentName;
...
const logoFiles = generateComponents(logoDir, "Bx", "Logo");
const regularFiles = generateComponents(regularDir, "Bx", "Regular");
const solidFiles = generateComponents(solidDir, "Bx", "Solid");

// Generate index file
const indexContent = [
  ...logoFiles.map((svgFile: string) => {
    const componentName = svgFile
      .replace(".svg", "")
      .split(/[_&-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as BxLogo${componentName} } from './components/BxLogo${componentName}.vue'`;
  }),
  ...regularFiles.map((svgFile: string) => {
    const componentName = svgFile
      .replace(".svg", "")
      .split(/[_&-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as BxRegular${componentName} } from './components/BxRegular${componentName}.vue'`;
  }),
  ...solidFiles.map((svgFile: string) => {
    const componentName = svgFile
      .replace(".svg", "")
      .split(/[_&-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as BxSolid${componentName} } from './components/BxSolid${componentName}.vue'`;
  }),
].join("\n");

writeFileSync(join(boxIconsDir, "index.ts"), indexContent);
