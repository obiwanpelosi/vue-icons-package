...
const cssGgIconsDir = join(__dirname, "..");
const svgDir = join(cssGgIconsDir, "icons");
const outputDir = join(cssGgIconsDir, "components");
...
function addIconMetadata(componentName: string) {
  const componentPath = `./icons/css-gg-icons/components/${componentName}.vue`;
  const metadata = {
    name: componentName,
    group: "css-gg-icons",
    groupShortName: "cg",
...
const iconFiles = generateComponents(svgDir, "Cg");
...
const indexContent = [
  ...iconFiles.map((svgFile: string) => {
    const componentName = svgFile
      ...
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    return `export { default as Cg${componentName} } from './components/Cg${componentName}.vue'`;
  }),
].join("\n");
